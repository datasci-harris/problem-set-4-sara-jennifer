...
for _, row in active_2016.iterrows():
    ...
    for check_year in range(2017, 2020):
        hospital_status = data[(data["FAC_NAME"] == facility_name) & (data["year"] == check_year)]
...
print(f"There are {potential_merger_df.shape[0]} hospitals that fit the definition of potentially being a merger/acquisition")
```
...
filtered_valid_closure_df = valid_closure_df[~valid_closure_df.index.isin(remaining_hospitals_df.index)]

print(f"There are {filtered_valid_closure_df.shape[0]} hospitals left after correcting for potential mergers/acquisitions")
```
...
# Filtering
filtered_valid_closure_df_sorted = filtered_valid_closure_df.sort_values(by="FAC_NAME")
